...
export default function InsightsPage() {
  ...
  const { spendValues, monthLabels } = useMemo(() => {
    ...
    return { spendValues: allSpendValues.slice(start), monthLabels: allMonthLabels.slice(start) }
  }, [range])
  ...
  const { barLabels, barSeries } = useMemo(() => {
    const start = Math.max(0, barAllLabels.length - Math.min(range, 6))
    return { barLabels: barAllLabels.slice(start), barSeries: barAllSeries.map(s => ({ ...s, data: s.data.slice(start) })) }
  }, [range])

  return (
    <Layout>
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-sm font-semibold text-gray-900 dark:text-warm-gray-100">Phân tích</h2>
      </div>
      <div className="grid grid-cols-12 gap-4 md:gap-5 xl:gap-6">
        <div className="col-span-12 sm:col-span-6 lg:col-span-3"><BalanceCard /></div>
        <div className="col-span-12 sm:col-span-6 lg:col-span-3"><StatsCard title="Khách hàng" value="20,032" subtitle="+2% tăng trưởng" icon={Users} color="#16a34a" /></div>
        <div className="col-span-12 sm:col-span-6 lg:col-span-3"><StatsCard title="Đơn hàng" value="350" subtitle="+5 hôm nay" icon={ShoppingCart} color="#60a5fa" /></div>
        <div className="col-span-12 sm:col-span-6 lg:col-span-3"><StatsCard title="Doanh thu" value="$3,540" subtitle="Tháng này" icon={DollarSign} color="#f59e0b" /></div>
        ...
        <div className="col-span-12 lg:col-span-7 rounded-xl bg-white dark:bg-warm-gray-800 border border-gray-100 dark:border-warm-gray-700 p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex gap-1 bg-gray-100 dark:bg-warm-gray-800 rounded p-0.5">
              {[6, 12].map(r => (
                <button key={r} onClick={() => setRange(r)} className={`px-2 py-1 text-xs rounded ${range === r ? 'bg-white dark:bg-warm-gray-700 shadow-sm' : 'text-gray-600 dark:text-warm-gray-300'}`}>{r}T</button>
              ))}
            </div>
          </div>
        ...
        <div className="col-span-12 lg:col-span-7"><TransactionsList /></div>
        <div className="col-span-12 lg:col-span-5"><DataInfo /></div>
      </div>
    </Layout>
  )
}